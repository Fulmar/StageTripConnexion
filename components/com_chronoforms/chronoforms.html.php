...
class HTML_ChronoForms {
	public static function processView($form){
		...
		if(empty($form->form_name)){
            ...
            $show_page_heading = $app->getPageParameters('com_chronoforms')->get('show_page_heading');
            if($show_page_heading == 1) {
                $page_title = $app->getPageParameters('com_chronoforms')->get('page_title');
                echo '<h2 class="title-form">' . $page_title . '</h2>';
            }
